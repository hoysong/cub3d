...
- [화가 알고리즘](https://en.wikipedia.org/wiki/Painter%27s_algorithm)을 이용한 원거리→근거리 순서의 벽 렌더링
...
플레이어의 시야각을 일정 간격으로 나눠 광선을 발사하고, 각 방향에서 처음
충돌하는 벽을 찾습니다. 같은 벽과 텍스처에 연속해서 충돌한 결과는 하나의 벽
구간으로 묶어 화면 좌표로 변환합니다.

화면에 그리기 전 벽 구간을 원거리부터 근거리 순서로 정렬하고 차례대로
렌더링합니다. 이는 [화가 알고리즘](https://en.wikipedia.org/wiki/Painter%27s_algorithm)을
적용한 것으로, 화면 영역이 겹칠 때 가까운 벽이 먼 벽을 덮어쓰게 합니다. 각 벽
구간에는 방향과 좌표에 맞는 XPM 텍스처의 세로 영역을 그립니다.
